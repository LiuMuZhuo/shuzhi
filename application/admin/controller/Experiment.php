...
class Experiment extends Common{
    public function index(){
        $experiment_model = new \app\config\model\ExperimentM();
...
//        return '<style type="text/css">*{ padding: 0; margin: 0; } .think_default_text{ padding: 4px 48px;} a{color:#2E5CD5;cursor: pointer;text-decoration: none} a:hover{text-decoration:underline; } body{ background: #fff; font-family: "Century Gothic","Microsoft yahei"; color: #333;font-size:18px} h1{ font-size: 100px; font-weight: normal; margin-bottom: 12px; } p{ line-height: 1.6em; font-size: 42px }</style><div style="padding: 24px 48px;"> <h1>:)</h1><p> ThinkPHP V5<br/><span style="font-size:30px">十年磨一剑 - 为API开发设计的高性能框架</span></p><span style="font-size:22px;">[ V5.0 版本由 <a href="http://www.qiniu.com" target="qiniu">七牛云</a> 独家赞助发布 ]</span></div><script type="text/javascript" src="http://tajs.qq.com/stats?sId=9347272" charset="UTF-8"></script><script type="text/javascript" src="http://ad.topthink.com/Public/static/client.js"></script><thinkad id="ad_bd568ce7058a1091"></thinkad>';
    ...
    public function form_text(){
        ...
        $experiment_model = new \app\config\model\ExperimentM();
        if ($tip==0){
            $tip = Request::instance()->get('tip',0);
            if ($tip==0){
...
//                $data['state'] = 1;
                $data['status_grade'] = Request::instance()->post('status_grade',1);
                $data['status_react'] = Request::instance()->post('status_grade',1);
                $data['uploader'] = Request::instance()->post('uploader','');
    ...
    public function form_video(){
        ...
        if ($tip==0){
            ...
            //增加实验示例程序
            $file_code = request()->file('code');
            $algo_picture = request()->file('algo_picture');
//            var_dump($file_video);exit();
            if($file_picture){
...
//                    $this->success('上传成功','/admin.php/admin/experiment');
            ...
            if($file_code){
                $info = $file_code->move(ROOT_PATH . 'public' . DS. 'uploads');
                if($info){
                    $data['code_file'] = '/public/uploads/'.$info->getSaveName();
                }else{
                    $this->error('上传失败，可能出现的原因：'.$file_code->getError());
                }
            }
//            if($algo_picture){
//                $info = $algo_picture->move(ROOT_PATH . 'public' . DS. 'uploads');
//                if($info){
//                    $data['algo_picture1'] = '/public/uploads/'.$info->getSaveName();
//                }else{
//                    $this->error('上传失败，可能出现的原因：'.$algo_picture->getError());
//                }
//            }
    ...
    public function delete(){
        ...
        $experiment = new \app\config\model\ExperimentM();
